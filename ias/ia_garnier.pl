...
% joue_coup(+BoardTopDown, +Joueur, -Col)
% BoardTopDown : liste de colonnes du haut vers le bas (format du moteur)
% Joueur : 'x' ou 'o'
% Col : colonne choisie (numéro 1..7)
%
% Cette fonction doit afficher dans stdout le numéro de colonne à jouer.
joue_coup(Board, Joueur, Col) :-
    aiMinimax(Board, Joueur, Col),
    format('~w~n', [Col]),
    flush_output.
